...
def parse_g16_employment(series_dims: list, series_data: dict, region_id: str,
    ...
    for v in lfsp_dim["values"]:
        ...
        # Total labour force
            ...
            _total_lf_code = code  # captured for completeness; not summed
        # Percentage codes (not counts — skip for summing)
        elif label.startswith("%"):
            if "participation" in label:
                _pct_participation_code = code  # captured for completeness; not summed
            elif "unemployment" in label:
                _pct_unemployment_code = code  # captured for completeness; not summed
