...
def generate(state: str, election_id: int, db_path: Path) -> None:
    """Generate and print the JS constant for a single state/election."""
...
def main() -> None:
    ...
    if args.state == "all":
        for state, election_id in DEFAULT_ELECTIONS.items():
            yr = args.year or election_id
            print(f"# ═══════════════════ {state.upper()} {yr} ═══════════════════")
            generate(state, yr, args.db)
    else:
        ...
        if election_id is None:
            ...
            sys.exit(1)
        generate(state, election_id, args.db)
